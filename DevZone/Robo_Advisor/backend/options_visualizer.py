...
class OptionsVisualizer:
    def __init__(self, options_data=None, time_frame=None):
        ...
        """
        self.options_data = options_data.copy() if options_data is not None else pd.DataFrame()
    ...
    def prepare_data(self):
        ...
        self.options_data.loc[:, 'Expiration'] = pd.to_datetime(self.options_data['Expiration'], errors='coerce')
        self.options_data.loc[:, 'Type'] = self.options_data['Type'].str.upper()

        numeric_columns = ['Strike', 'Volume', 'Open Interest', 'Last Price']
        for col in numeric_columns:
            self.options_data.loc[:, col] = pd.to_numeric(self.options_data[col], errors='coerce')
    ...
    def filter_data_by_time_frame(self):
        ...
        return self.options_data[self.options_data['Expiration'] <= end_time].copy()
    ...
    def plot_options_chain(self):
        ...
        calls = self.filtered_data[self.filtered_data['Type'] == 'CALL'].copy()
        puts = self.filtered_data[self.filtered_data['Type'] == 'PUT'].copy()
    ...
    def plot_option_history(self, option_history, option_type):
        ...
        """
        logger.info("Creating option history visualization")
        ...
        logger.info("Option history visualization created successfully")
        return fig